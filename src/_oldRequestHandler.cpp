...
std::string readFile(const std::string& filepath, size_t clientMaxBodySize)
{
    std::ifstream file(filepath.c_str(), std::ios::binary);
    if (!file)
        return "";

    // Create a string stream to hold the content
    std::ostringstream content;
    ...
    {
        size_t bytesRead = file.gcount();  // Number of bytes actually read

        // Check if adding these bytes would exceed the clientMaxBodySize
        if (totalRead + bytesRead > clientMaxBodySize)
        {
            // Only read as much as the max body size allows
            bytesRead = clientMaxBodySize - totalRead;
            content.write(buffer, bytesRead); // Write only the allowed amount
            totalRead = clientMaxBodySize;    // We hit the max size
            break;
        }

        content.write(buffer, bytesRead); // Append the data to the content
        totalRead += bytesRead;
    }
...
void			RequestHandler::handle_get(std::string& path, int client_socket, ConfigParser& config)
{
	...
	std::string content = readFile(filepath, config.getbodySize());
	std::string response;
	if (!content.empty())
		response = generateResponse(200, content, "text/html");
	else
	{
		std::string errorpath = config.getRoot() + config.getErrorPage();
		std::string errorcontent = readFile(errorpath, config.getbodySize());
		if (!errorcontent.empty())
			response = generateResponse(404, errorcontent, "text/html");
		else
